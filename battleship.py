...
class Player:
    ...
    'SHIP_FIVE':  ['◀', '◀', '◀', '◀', '◀']}
        ...
        self.board = [self.x_axis] + [[self.y_axis[i]]+ self.rows for i in \
        range(len(self.y_axis))]
        self.guess_board = [self.x_axis] + [[self.y_axis[i]]+ self.rows for i \
        in range(len(self.y_axis))]
    def display_board(self):
        ...
        printable_board[-1] = '10' + printable_board[-1][3:]
    ...
    def display_guess_board(self):
        ...
        printable__guess_board[-1] = '10' + printable__guess_board[-1][3:]
        ...
        try:
            ...
            if orientation[0].lower() == 'h':
                if x+ship_length > len(self.board[y]):
                    raise IndexError()
                try:
                    if '◀' in self.board[y-1][x:(x+ship_length)] or '▼' in \
                    self.board[y][x:(x+ship_length)]:
                        delayed_type("SHIP TOO CLOSE! CHOOSE AGAIN!", .01)
                        self.get_coords(ship)
                    elif '◀' in self.board[y][x:(x+ship_length+1)] or '▼' in \
                    self.board[y-1][x:(x+ship_length+1)]:
                        delayed_type("SHIP ALREADY THERE! CHOOSE AGAIN!", .01)
                        self.get_coords(ship)
                    elif '◀' in self.board[y+1][x:(x+ship_length+1)] or '▼' in \
                    self.board[y+1][x:(x+ship_length+1)]:
                        delayed_type("SHIP TOO CLOSE! CHOOSE AGAIN!", .01)
                        self.get_coords(ship)
                    else:
            ...
            if orientation[0].lower() == 'v':
                if y+ship_length > len(self.board):
                    raise IndexError()
                try:
                    if '▼' in [self.board[y+h][x-1] for h in range(-1, ship_length+1)] or \
                    '◀' in [self.board[y+h][x-1] for h in range(ship_length)]:
                        delayed_type("SHIP TOO CLOSE! CHOOSE AGAIN!", .01)
                        self.get_coords(ship)
                    elif '▼' in [self.board[y+h][x] for h in range(-1, ship_length, +1)] or \
                    '◀' in [self.board[y+h][x] for h in range(ship_length)]:
                        delayed_type("SHIP ALREADY THERE! CHOOSE AGAIN!", .01)
                        self.get_coords(ship)
                    elif '▼' in [self.board[y+h][x+1] for h in range(1, ship_length, +1)] or \
                    '◀' in [self.board[y+h][x+1] for h in range(ship_length)]:
                        delayed_type("SHIP TOO CLOSE! CHOOSE AGAIN!", .01)
                        self.get_coords(ship)
                    else:
    ...
    def get_coords(self, ship):
        ...
        coordinate = input()
        while (coordinate == '') or (coordinate[0].upper() not in \
            self.x_axis) or (coordinate[1:] not in self.y_axis):
    ...
    def targeting(self, other_ship):
        ...
        target = input()
        while target == '' or (target[0].upper() not in self.x_axis) or \
        (target[1:] not in self.y_axis):
    ...
    def check_status(self, other_ship):
        ...
        for i in other_ship.fleet_locations:
            ...
            if ship_vertical:
                other_ship.fleet[i] = \
                [other_ship.board[ship[0][0]+i][ship[1]] for i in \
                range(len(self.fleet[i]))]
            if ship_horizontal:
                other_ship.fleet[i] = \
                other_ship.board[ship[0]][ship[1][0]:ship[1][1]]
            if all('X' == integrity for integrity in other_ship.fleet[i]) \
